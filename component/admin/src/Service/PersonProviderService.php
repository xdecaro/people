...
final class PersonProviderService
{
    ...
    public function searchPeople(array $filters = [], int $limit = 50, bool $sensitive = false): array
    {
        ...
        if ($search !== '') {
            ...
            $query->where('(' . $this->db->quoteName('p.display_name') . ' LIKE :s1 OR ' . $this->db->quoteName('p.email') . ' LIKE :s2)')
                ->bind(':s1', $like)
                ->bind(':s2', $like);
        }
    ...
    {
        $columns = [
            'p.id', 'p.uuid', 'p.user_id', 'p.display_name', 'p.first_name', 'p.last_name',
            'p.email', 'p.phone', 'p.whatsapp', 'p.state', 'p.access', 'p.language',
        ];

        if ($sensitive) {
            $columns = array_merge($columns, [
                'p.birth_date', 'p.sex', 'p.disability_status', 'p.birth_place', 'p.nationality_code',
                'p.tax_identifier', 'p.address_line', 'p.address_number', 'p.postal_code', 'p.city',
                'p.region', 'p.country_code', 'p.social_instagram', 'p.social_facebook', 'p.social_linkedin',
                'p.social_tiktok', 'p.social_telegram', 'p.social_x', 'p.social_youtube', 'p.website_url', 'p.notes',
            ]);
        }
