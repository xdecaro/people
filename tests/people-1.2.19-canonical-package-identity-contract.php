...
if ($version !== '1.2.19') {
    fwrite(STDERR, "People canonical package/identity-details release must be 1.2.19.\n");
    exit(1);
}
...
foreach ($requiredFiles as $path) {
    if (!is_file($path)) {
        fwrite(STDERR, "Missing People 1.2.19 canonical file: {$path}\n");
        exit(1);
    }
}
...
$checks = [
    ...
    [$feed, 'pkg_people_1.2.19.zip', 'People update feed must publish pkg_people_1.2.19.zip.'],
...
echo "People 1.2.19 canonical package and identity-details contract OK\n";
