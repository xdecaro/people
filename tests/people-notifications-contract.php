...
$expect((bool) preg_match('/name="notification_recipient_user_id"[^>]*type="user"/i', $config), 'People options must expose a Joomla user recipient field.');
...
foreach ([
    ...
    '$possibleDuplicate = $this->warnPossibleDuplicate(',
    'notifyPersonCreated(',
    'notifyPersonUpdated(',
    ...
    $expect(str_contains($model, $marker), "PersonModel notification contract missing marker: {$marker}");
}
